...
let usuarioEspecializado = localStorage.getItem('correo');
console.log(usuarioEspecializado);

async function postAsistenciaEspecializado(usuarioEspecializado) {
  valor = { usuarioEspecializado: usuarioEspecializado }
  const res = await fetch('/listarSiniestroAsistencia/recibirEspecializado', {
    method: 'POST',
...
document.addEventListener("DOMContentLoaded", async function renderAsistencia() {
  ...
  let response = await postAsistenciaEspecializado(usuarioEspecializado);
  ...
  for (let i = 0; i < response.length; i++) {
    ...
    //usuarioSolicitante
    let td_usuarioSolicitante = document.createElement("td");
    let texto_usuarioSolicitante = document.createTextNode
      (response[i].usuarioSolicitante);
    td_usuarioSolicitante.appendChild(texto_usuarioSolicitante);
    tr.appendChild(td_usuarioSolicitante);
    table.appendChild(tr);
